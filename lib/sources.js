...
function pakistanConfidence(paper){
  ...
  // A genuinely unambiguous term in the TITLE (Lahore, Karachi, Islamabad,
  // "Pakistan" itself, etc.) is trusted outright — authors title papers after
  // their real study area.
  if(textHasAny(title, UNAMBIGUOUS_PK_TERMS)){
    return paper.verifiedPk ? 'verified' : 'keyword';
  }

  // Title doesn't confirm it (including a bare "Punjab"/"Balochistan" in the
  // title alone, which isn't proof by itself). Check the full text.
  const fullText = title + ' ' + (paper.abstract||'') + ' ' + (paper.venue||'');
  const hasUnambiguous = textHasAny(fullText, UNAMBIGUOUS_PK_TERMS);
  const hasAmbiguousOnly = !hasUnambiguous && textHasAny(fullText, AMBIGUOUS_PK_TERMS);
  const mentionsPk = hasUnambiguous || hasAmbiguousOnly;
  if(!mentionsPk) return null;

  // An abstract/venue-only mention (title silent) is NOT reliable proof of
  // study area: large multi-country/global studies (public health, climate,
  // etc.) routinely list Pakistan among dozens of nations without it being
  // the actual focus, and no fixed conflicting-country list can ever cover
  // every one of those other nations (Brazil, Nigeria, Sweden... aren't
  // Pakistan's neighbors, so they'd never get flagged, yet their presence is
  // just as telling as India's or Iran's would be).
  //
  // OpenAlex gives us real author-institution data, so for that source we use
  // it instead of guessing: an abstract-only mention only counts if a real
  // Pakistan-affiliated author backs it up.
  if(paper.source === 'OpenAlex'){
    return paper.verifiedPk ? 'verified' : null;
  }

  // Other sources expose no institution data at all, so the conflicting-
  // country list remains the best available (imperfect) fallback signal.
  const hasConflict = textHasAny(fullText, CONFLICTING_COUNTRY_TERMS);
...
function buildRequests(state){
  ...
  if(state.s2HasMore){
    ...
    let url = `https://api.semanticscholar.org/graph/v1/paper/search?query=${encodeURIComponent(kw)}&fields=${fields}&limit=${PAGE_S2}&offset=${state.s2Offset}`;
    if(year) url += `&year=${year}`;
    reqs.s2 = { url };
  }

  if(state.crHasMore){
    let url = `https://api.crossref.org/works?query=${encodeURIComponent(kw)}&rows=${PAGE_CR}&offset=${state.crOffset}&select=title,author,published,DOI,container-title,URL,abstract,is-referenced-by-count`;
    if(year){
  ...
  if(state.oaHasMore){
    ...
    let url = `https://api.openalex.org/works?search=${encodeURIComponent(kw)}&select=${oaSelect}&per-page=${PAGE_OA}&page=${state.oaPage}&mailto=talaashgis@app`;
    if(process.env.OPENALEX_API_KEY) url += `&api_key=${process.env.OPENALEX_API_KEY}`;
    if(year){
      const [fromY, toY] = yearRange(year);
      if(fromY) url += `&filter=from_publication_date:${fromY}-01-01,to_publication_date:${toY}-12-31`;
    }
    reqs.oa = { url };
  }
